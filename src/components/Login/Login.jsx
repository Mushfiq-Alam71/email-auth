...
const Login = () => {
    ...
    const handleLogin = e => {
        ...
        if (password.length < 6) {
            ...
            setLoginError('password must contain uppercase letters');
            return;
        }
    ...
    const handleResetPassword = () => {
        ...
        if (!email) {
            console.log('provide email', email.current.value);
        } else if (!/^[\w\.-]+@[a-zA-Z\d\.-]+\.[a-zA-Z]{2,}$/.test(email)) {
            console.log('pls write valid email');
        }
        ...
        <div >
            ...
            <div className="w-full max-w-md p-8 space-y-3 rounded-xl   border-2 ">
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="space-y-1 text-sm">
                        ...
                        <div className="relative">
                            ...
                            />
                            <span
                                className="text-xl absolute top-3 right-3 text-black"
                                onClick={() => SetShowPassword(!showPassword)}>
                                {showPassword ? <FaEyeSlash /> : <FaEye />}
                            </span>
                        </div>
                        <div className="flex justify-end text-xs dark:text-white">
                            <a onClick={handleResetPassword} rel="noopener noreferrer" href="#" className="hover:underline text-lg">Forgot Password?</a>
                        </div>
                    </div>
                ...
                <p className="text-xs text-center sm:px-6 dark:text-white">Don&#39;t have an account?
                    <NavLink to='/register' rel="noopener noreferrer" href="#" className="hover:underline dark:text-white pl-1">Sign in</NavLink>
                </p>
            </div>
        </div>
    );
